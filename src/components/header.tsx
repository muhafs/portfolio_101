	...
	return (
		<motion.header className="fixed top-0 md:mt-12 md:mr-12 right-0 z-20">
			<Transition className="fixed md:top-8 top-6 md:left-8 left-6 z-30 ">
				<Link href={'/'}>
					<TextReveal className="font-semibold ">Muhafs.</TextReveal>
				</Link>
			</Transition>
			<motion.div initial={false} animate={isActive ? 'open' : 'closed'} variants={variants} className="absolute top-0 right-0 md:-top-6 md:-right-6 w-dvw md:w-[480px] h-dvh md:h-[calc(100dvh_-_2.5rem)] bg-primary">
				{isActive && (
					<nav className="flex justify-between flex-col w-full h-full px-10 pt-[100px] pb-[50px]">
						...
						<motion.div className="flex flex-wrap">
							{social.map((link, i) => {
								...
								return (
									<MotionLink href={url} target="_blank" className=" w-1/2 mt-1 text-background" variants={slideIn} custom={i} initial="initial" animate="enter" exit="exit" key={_id}>
...
function Button({ isActive, toggleMenu }: { isActive: boolean; toggleMenu: () => void }) {
	return (
		<div className="absolute md:top-0 top-4 right-4 md:right-0 w-[100px] h-10 rounded-full overflow-hidden cursor-pointer">
			<motion.div className="relative w-full h-full" animate={{ top: isActive ? '-100%' : '0%' }} transition={{ duration: 0.5, type: 'tween', ease: [0.76, 0, 0.24, 1] }}>
				<motion.div
