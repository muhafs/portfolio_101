		...
		<motion.section className="relative" id="contact">
			<AnimatePresence initial={false}>
				{status !== 'IDLE' && (
					<motion.li
						...
						className={cn('fixed top-4 right-4 p-2 px-4 w-[300px] z-50 h-16 rounded-xl bg-white flex items-center', status === 'ERROR' ? 'bg-red-500' : status === 'DONE' ? 'bg-green-400' : '')}>
			...
			<div className="p-4 md:p-8 md:px-16">
				<SectionHeading className="">
					<SlideIn className="text-white/40">Interested in talking,</SlideIn> <br /> <SlideIn>let’s do it.</SlideIn>
				</SectionHeading>
				<div className="grid md:grid-cols-2 gap-10 md:pt-16">
					<form className="space-y-4" onSubmit={handleSubmit}>
						...
						<div>
							<Transition>
								<motion.button whileHover="whileHover" initial="initial" className="border border-white/30 px-8 py-2 rounded-3xl relative overflow-hidden" type="submit">
									<TextReveal className="uppercase">{status === 'SENDING' ? 'Sending...' : 'discuss project'}</TextReveal>
								</motion.button>
							</Transition>
						</div>
					</form>
			...
			<footer className="flex items-center justify-between md:px-8 px-2 py-4 text-sm">
				<Transition>
					<div>&copy; {new Date().getFullYear()} Muhafs.</div>
				</Transition>
				<Transition>
					<p>
						developed by @
						<Link href={'https://github.com/muhafs'} className="hover:underline">
							Muhafs
						</Link>
					</p>
				</Transition>
